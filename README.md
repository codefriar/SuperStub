...
Apex provides a system interface calling `StubProvider` - [you can learn more about it here.](https://developer.salesforce.com/docs/atlas.en-us.apexref.meta/apexref/apex_interface_System_StubProvider.htm) There's even advanced instructions for [building a mocking and stubbing framework with the StubProvider interface](https://developer.salesforce.com/docs/atlas.en-us.232.0.apexcode.meta/apexcode/apex_testing_stub_api.htm) 

While these documents explain what the StubProvider does, the examples given lend themselves to a one-off Stub classes. For obvious reasons this isn't brilliant, and so I and others have set out to build a reusable Stubbing framework. There are other reusable frameworks out there, but this one is 'mine'. Part of what makes this kind of framework difficult is the `API`, or the interface developers use when utilizing this kind of library. Much thought has been put into `API` of Super-Stub. Specifically, I've tried to make sure this libraries methods names are intuitive and easy to understand. 

It's built as a series of three classes: `Stub`, `MockedMethod` and `MethodSignature`. Each of these classes has an inner class named `Builder`. The builder class exposes a fluent interface for progressively and intuitively building Stub objects and Mocked methods.

## Who's it for?
🧑‍💻👩🏾‍💻 - Apex developers who write unit tests! (aka: All Apex developers) If you write unit tests, and want to make them faster, and less prone to intermittent failures then you'll want to utilize Stubing.  
...
- Mock: A Mock referrs to a specific function who's implementation is overridden by a Stub. Stub's contain at least one Mock. When the Stub is used, and the tested code invokes the Mock, the Stub's implementation is used, instead of the normal implementation. In Super-Stub, Mocks are represeted by the `MockedMethod` class. 
- Dependency Injection: This referrs to the practice of 'injecting' one object into another for use. In practice, this often looks like having an Object's constructor require an instance of another object. Contrast this idea to the idea of having an object create it's own instance of a given object. You can *only use the StubProvider interface, and Super-Stub with code that uses Dependency Injection.*
...
There are other Stubbing and Mocking frameworks for Apex. Other's have outright asked me why I reinvented the wheel. Honestly, because none of the others met my needs. Nevertheless, Here's a list of other Stub frameworkds for Apex: (Feel free to submit a PR to have your library listed.)
